        ...
        while(fscanf(oldf, "%d %s %d/%d/%d %d %s %s %lf %s %f %d/%d/%d" , &add.acc_no,add.name,&add.dob.month,&add.dob.day,&add.dob.year,&add.age,add.address,add.citizenship,&add.phone_no,add.acc_type,&add.amt,&add.deposit.month,&add.deposit.day,&add.deposit.year)!=EOF)
            if(add.acc_no == check.acc_no){
                ...
                if(strcmp(add.acc_type, "Fixed1")==0){
                    time=1;
                    rate=5;
                    SI=interest(time, add.amt, rate);
                    printf("You will get %0.2f as an interest on %d/%d/%d:\n ", SI, add.deposit.day, add.deposit.month, add.deposit.year+1);
                }
                if(strcmp(add.acc_type, "Fixed2")==0){
                    time=1;
                    rate=7;
                    SI=interest(time, add.amt, rate);
                    printf("You will get %0.2f as an interest on %d/%d/%d:\n ", SI, add.deposit.day, add.deposit.month, add.deposit.year+2);
                }
                if(strcmp(add.acc_type, "Fixed3")==0){
                    time=1;
                    rate=9;
                    SI=interest(time, add.amt, rate);
                    printf("You will get %0.2f as an interest on %d/%d/%d:\n ", SI, add.deposit.day, add.deposit.month, add.deposit.year+3);
                }
                if(strcmp(add.acc_type, "Saving")==0){
                    time=1;
                    rate=6;
                    SI=interest(time, add.amt, rate);
                    printf("You will get %0.2f as an interest on %d of every month:\n ", interest, add.deposit.day);
                }
            ...
            while(fscanf(oldf, "%d %s %d/%d/%d %d %s %s %lf %s %f %d/%d/%d" , &add.acc_no,add.name,&add.dob.month,&add.dob.day,&add.dob.year,&add.age,add.address,add.citizenship,&add.phone_no,add.acc_type,&add.amt,&add.deposit.month,&add.deposit.day,&add.deposit.year)!=EOF){
                ...
                test=1;
                    ...
                    if(strcmp(add.acc_type, "Fixed1")==0){
                        ...
                        rate=5;
                        SI=interest(time, add.amt, rate);
                        printf("You will get %0.2f as an interest on %d/%d/%d:\n ", interest, add.deposit.day, add.deposit.month, add.deposit.year+1);
                    }
                    if(strcmp(add.acc_type, "Fixed2")==0){
                        time=1;
                        rate=7;
                        SI=interest(time, add.amt, rate);
                        printf("You will get %0.2f as an interest on %d/%d/%d:\n ", interest, add.deposit.day, add.deposit.month, add.deposit.year+2);
                    }
                    if(strcmp(add.acc_type, "Fixed3")==0){
                        time=1;
                        rate=9;
                        SI=interest(time, add.amt, rate);
                        printf("You will get %0.2f as an interest on %d/%d/%d:\n ", interest, add.deposit.day, add.deposit.month, add.deposit.year+3);
                    }
                    if(strcmp(add.acc_type, "Saving")==0){
                        time=1;
                        rate=6;
